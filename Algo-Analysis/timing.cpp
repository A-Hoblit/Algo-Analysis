...
int main() {
  ...
  std::fstream fout;
  std::string name = "test_"+std::to_string(5)+"_"+std::to_string(20)+".csv";
  fout.open(name.c_str(), std::ios::out | std::ios::app);
  //n is the maxmium input size to test if divisible by 5
  for(unsigned int i =5; i<=25; i+=1) {
    std::string input[2];
    std::mt19937 rng(i);

    // random string taken from below
    std::uniform_int_distribution<int> rand_letter('a', 'z');
    while (input[0].size() < i) {
      input[0].push_back(rand_letter(rng));

      // random date taken from below
      ...
      assert(25 >= date_str.size()); // the reason for MIN_N
      size_t padding_chars = 25 - date_str.size();
      input[1].assign(padding_chars, ' ');
      input[1] += date_str;

      Timer timer;
      double elapsed; 
        //argv is amount of times to test at input size argc
        for(unsigned int j=0; j<10; j++) {
        
          timer.reset();
          algorithms::run_length_encode(input[0]);
          elapsed = timer.elapsed();
          fout<<i<<","<<elapsed;

          timer.reset();
          algorithms::longest_frequent_substring(input[0], 3);
          elapsed = timer.elapsed();
          fout<<","<<elapsed;

          timer.reset();
          algorithms::reformat_date(input[1]);
          elapsed = timer.elapsed();
          fout<<","<<elapsed<<"\n";
        }
      }
